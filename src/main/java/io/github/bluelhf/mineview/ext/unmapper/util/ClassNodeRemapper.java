...
public class ClassNodeRemapper {
	...
	public void remap(ClassNode cn, boolean removeLocals) {
		remapMethods(cn, removeLocals);
		remapFields(cn);
	...
	private void remapMethods(ClassNode cn, boolean removeLocals) {
		if (cn.methods == null)
			return;
	...
	private void setTypeFields(Type type) {
		try {
			...
			String buf = (String) f_buf.get(type);
			String oldBuf = getOldClassName(buf);
			f_buf.set(type, oldBuf);
